...
export function ExploradorAnalise({ snapshotId }: ExploradorAnaliseProps) {
  ...
  return (
    <section aria-labelledby="exploracao-titulo" className="mt-8">
      ...
      <ExploradorConteudo key={arquivoSelecionado ?? 'sem-selecao'} snapshotId={snapshotId} arquivoSelecionado={arquivoSelecionado} onSelecionarArquivo={selecionarArquivo} t={t} />
    </section>
  )
}
...
function ExploradorConteudo({ snapshotId, arquivoSelecionado, onSelecionarArquivo, t }: { snapshotId: string; arquivoSelecionado: string | null; onSelecionarArquivo: (arquivo: string) => void; t: (key: string) => string }) {
  ...
  return <>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(18rem,24rem)_minmax(0,1fr)]">
      <div className={modoMobile === 'arquivos' ? 'block' : 'hidden lg:block'}><NavegadorArquivos arvore={arvore.data} caminho={caminho} isLoading={arvore.isLoading} isError={arvore.isError} onAbrirPasta={setCaminho} onAbrirArquivo={onSelecionarArquivo} onVoltar={() => setCaminho((atual) => pastaPai(atual))} onTentarNovamente={() => void arvore.refetch()} /></div>
      <div className={modoMobile === 'relacoes' ? 'block' : 'hidden lg:block'}><PainelRelacoes arquivo={arquivoSelecionado} relacoes={relacoes.data} isLoading={relacoes.isLoading} isError={relacoes.isError} erro={relacoes.error} onSelecionar={onSelecionarArquivo} />{arquivoSelecionado ? <button type="button" onClick={() => setModoMobile('arquivos')} className="mt-4 inline-flex min-h-11 items-center rounded-xl px-3 text-sm font-semibold text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring lg:hidden">{t('voltarArquivos')}</button> : null}</div>
    </div>
  </>
}
