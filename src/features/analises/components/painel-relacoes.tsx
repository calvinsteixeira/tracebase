...
export function PainelRelacoes({ arquivo, relacoes, isLoading, isError, erro, onSelecionar, onTentarNovamente }: PainelRelacoesProps) {
  ...
  if (!arquivo) return <section aria-labelledby="relacoes-titulo" className="flex min-h-[24rem] items-center justify-center rounded-3xl border border-dashed border-border bg-card/50 p-6 text-center"><div className="max-w-sm"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">{t('relacoes')}</p><h2 id="relacoes-titulo" className="mt-3 text-xl font-semibold">{t('semSelecao')}</h2></div></section>
  if (isLoading) return <section aria-labelledby="relacoes-titulo" className="flex min-h-[24rem] items-center justify-center rounded-3xl border border-border bg-card p-6"><div className="flex items-center gap-3 text-sm text-muted-foreground"><LoaderCircle aria-hidden="true" className="size-5 animate-spin motion-reduce:animate-none" />{t('carregandoRelacoes')}</div></section>
  if (isError || !relacoes) return <section aria-labelledby="relacoes-titulo" className="flex min-h-[24rem] items-center justify-center rounded-3xl border border-border bg-card p-6 text-center"><div className="max-w-sm"><p id="relacoes-titulo" className="text-sm text-muted-foreground">{erro instanceof ErroExploracaoAnaliseCliente && erro.codigo === 'SNAPSHOT_NAO_ENCONTRADO' ? t('erroSnapshot') : t('erroRelacoes')}</p><button type="button" onClick={onTentarNovamente} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl border border-border px-4 text-sm font-semibold transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><RotateCw aria-hidden="true" className="size-4" />{t('tentarNovamente')}</button></div></section>
