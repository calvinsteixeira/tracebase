...
- A Supabase CLI, instalada como dependência de desenvolvimento do projeto, é a ferramenta oficial para descobrir, ordenar, aplicar e registrar migrations. Não crie executor próprio, leitura manual de arquivos SQL ou histórico paralelo em `public.schema_migrations`; o histórico pertence a `supabase_migrations.schema_migrations`.
...
- Scripts locais podem orquestrar Docker, healthcheck, proteção contra destinos remotos e inicialização do Next.js, mas devem delegar migrations à Supabase CLI instalada no projeto.
